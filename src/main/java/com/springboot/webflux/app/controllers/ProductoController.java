...
public class ProductoController {
	...
	public Mono<ResponseEntity<Producto>> crearConFoto(Producto producto, @RequestPart FilePart file) {
		...
		return file.transferTo(new File(path + producto.getFoto())).then(service.save(producto)).map(p -> ResponseEntity.created(URI.create("/api/productos/".concat(p.getId())))
	...
	// La forma mas facil de implementar ya que se guarda en el response body del
	// controlador de forma automatica
	/*
	 * @GetMapping private Flux<Producto> listar(){ return service.findAll(); }
	 */
